...
#endif
        ESP_LOGI(TAG, "Disconnected. Connecting to the AP again... retries: %d", retries);
        esp_wifi_connect();
    }
}
...
static void wait_for_wifi_connection() {/* Wait for Wi-Fi connection */
...
esp_err_t get_wifi_ssid(char *ssid, int max_len) {
    ...
    if (result == ESP_OK) {
        strncpy(ssid, (char *) conf.sta.ssid, max_len);
    }
    return result;
}
...
esp_err_t is_provisioned(bool *provisioned) {
    ...
    esp_err_t result = esp_wifi_get_config(WIFI_IF_STA, &wifi_cfg);
    if (result != ESP_OK) {
        return result;
    }
